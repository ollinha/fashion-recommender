...
if (isset($_POST['vote'])) {
	$vote = $_POST['voting'];
	mysql_query('INSERT INTO voting (user_id, voting_user_id, outfit) VALUES ('.$vote.', '.$user_id.', 3)');
}
?>
<div id="nav">
	<ul>
        ...
        <li>Galerie Arbeit</li>
    </ul>
</div>
<div id="content">
    <h1>Voting für den Anlass Arbeit</h1>
<?php
$voted_result = mysql_query('SELECT * FROM voting WHERE outfit=3 AND voting_user_id='.$user_id);
$voted_num = mysql_num_rows($voted_result);

if ($voted_num == 1) {
	echo '<p><span class="green">Sie haben Ihre Stimme für den Anlass Arbeit bereits vergeben.</span></p>';
...
?>
    ...
    <form action="start.php?action=gallery3" method="POST">
<?php
$result_voted = mysql_query('SELECT * FROM voting WHERE outfit=3 AND voting_user_id='.$user_id);
...
while ($row = mysql_fetch_array($result)) {
	...
	if ($user != $user_id) {
		...
		$outfit_result = mysql_query('SELECT history.product_id FROM history JOIN sessions ON sessions.id=history.session_id JOIN users ON users.id=sessions.user_id WHERE history.outfit=3 AND users.id='.$user);
