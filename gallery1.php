...
if (isset($_POST['vote'])) {
	$vote = $_POST['voting'];
	mysql_query('INSERT INTO voting (user_id, voting_user_id, outfit) VALUES ('.$vote.', '.$user_id.', 1)');
}
?>
<div id="nav">
	<ul>
        <li>Galerie Rendezvous</li>
        <a href="start.php?action=gallery2"><li>Galerie Party</li></a>
        <a href="start.php?action=gallery3"><li>Galerie Arbeit</li></a>
    </ul>
</div>
<div id="content">
    <h1>Voting für den Anlass Rendezvous</h1>
<?php
$voted_result = mysql_query('SELECT * FROM voting WHERE outfit=1 AND voting_user_id='.$user_id);
$voted_num = mysql_num_rows($voted_result);

if ($voted_num == 1) {
	echo '<p><span class="green">Sie haben Ihre Stimme für den Anlass Rendezvous bereits vergeben.</span></p>';
	$voted_row = mysql_fetch_array($voted_result);
	$voted_id = $voted_row['user_id'];
}
?>
    <p>Die folgenden Outfits wurden von den verschiedenen Nutzern für den Anlass Rendezvous gewählt. Bitte suchen Sie einen
    ...
    <form action="start.php?action=gallery1" method="POST">
<?php
$result_voted = mysql_query('SELECT * FROM voting WHERE outfit=1 AND voting_user_id='.$user_id);
...
while ($row = mysql_fetch_array($result)) {
	...
	if ($user != $user_id) {
		...
		$outfit_result = mysql_query('SELECT history.product_id FROM history JOIN sessions ON sessions.id=history.session_id JOIN users ON users.id=sessions.user_id WHERE history.outfit=1 AND users.id='.$user);
